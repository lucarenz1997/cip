...
class UIUtils:
    ...
    @staticmethod
    def show_selection_window_dropdown_3_levels(objects, title):
        selected_categories = []
        category_map = {}  # Dictionary to store mapping of TreeView item IDs to Category objects
        ...
        # Insert categories, subcategories, and third-level categories into the tree
        for category in objects:
            parent_id = tree.insert("", "end", text=category.name, tags=('deselected',))
            category_map[parent_id] = category  # Map the TreeView item ID to the Category object
            ...
            if category.subcategory:
                for subcat in category.subcategory:
                    ...
                    # Now add a third level, if it exists
                    if subcat.subcategory:
                        for subsubcat in subcat.subcategory:
                            subsubcat_id = tree.insert(subcat_id, "end", text=subsubcat.name, tags=('deselected',))
                            category_map[subsubcat_id] = subsubcat  # Map the third level category ID to the object
